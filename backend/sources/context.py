...
def selected_source_context(
    ...
    *,
    ...
    for index, source in enumerate(sources, start=1):
        ...
        source_text = str(source.get("extractedText") or "").strip()
        if not source_text and source.get("kind") == "image":
            source_text = "[Image source. Inspect the accompanying image input.]"
        elif not source_text:
            source_text = "[This source is stored but has no analyzable text.]"
