...
def safe_source_file_path(source: dict[str, Any]) -> Path | None:
    """Return a local source path only when it stays inside ``files_dir``."""
...
def read_source_bytes(source: dict[str, Any]) -> bytes | None:
    """Return source bytes from the configured local or object-store adapter."""
    metadata = source.get("metadata") or {}
    object_key = (
...
def source_image_input(source: dict[str, Any]) -> dict[str, str] | None:
    """Build an OpenAI-style image part for an owned notebook image source."""
    ...
    mime = str(source.get("mime") or mimetypes.guess_type(name)[0] or "image/png")
...
def image_inputs_for_source_ids(
    ...
    """Resolve selected owned image sources into provider-neutral payloads."""
