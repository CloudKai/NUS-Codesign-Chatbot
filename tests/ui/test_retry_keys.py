"""UI retention tests for Streamlit coach retry-key session state."""
...
def test_reuses_active_retry_without_storing_prompt_text():
    ...
    first = get_retry_key(
        state, thread_id="notebook-a", stage="focus", prompt="Private claim", now=NOW
    )
    replay = get_retry_key(
        state, thread_id="notebook-a", stage="focus", prompt="Private claim", now=NOW + 2
    )

    assert replay == first
    assert list(state[RETRY_KEYS_SESSION_KEY]) == [
        _scope("notebook-a", "focus", "Private claim")
    ]
...
def test_success_removal_and_expiry_produce_fresh_keys():
    ...
    state: dict[str, object] = {}
    first = get_retry_key(
        state, thread_id="notebook-a", stage="focus", prompt="Claim", now=NOW
    )
    remove_retry_key(
        state, thread_id="notebook-a", stage="focus", prompt="Claim", now=NOW + 1
    )
    after_success = get_retry_key(
        state, thread_id="notebook-a", stage="focus", prompt="Claim", now=NOW + 2
    )
...
def test_prunes_oldest_per_notebook_and_globally():
    ...
    assert _scope("notebook-a", "focus", "A 0") not in records
    assert _scope("notebook-a", "focus", "A 1") not in records
    ...
    assert _scope("notebook-0", "focus", "Claim") not in records
    assert _scope("notebook-3", "focus", "Claim") not in records
...
def test_notebook_switch_preserves_valid_entries_and_delete_purges_them():
    ...
    state: dict[str, object] = {}
    first = get_retry_key(
        state, thread_id="notebook-a", stage="focus", prompt="Claim A", now=NOW
    )
    get_retry_key(
        state, thread_id="notebook-b", stage="focus", prompt="Claim B", now=NOW + 1
    )

    assert get_retry_key(
        state, thread_id="notebook-a", stage="focus", prompt="Claim A", now=NOW + 2
    ) == first
    purge_notebook_retry_keys(state, "notebook-a", now=NOW + 2)
    records = state[RETRY_KEYS_SESSION_KEY]
    assert _scope("notebook-a", "focus", "Claim A") not in records
    assert _scope("notebook-b", "focus", "Claim B") in records
...
def test_migrates_exact_active_legacy_string_and_discards_other_legacy_state():
    ...
    state: dict[str, object] = {
        RETRY_KEYS_SESSION_KEY: {
            "notebook-a:focus:Claim A": active_key,
            "notebook-b:focus:Claim B": _key(),
    ...
    assert state[RETRY_KEYS_SESSION_KEY] == {
        _scope("notebook-a", "focus", "Claim A"): {
            "key": active_key,
            "notebook_id": "notebook-a",
            "scope_sha256": _scope("notebook-a", "focus", "Claim A"),
            "created_at": NOW,
        }
    }
...
def test_discards_malformed_expired_and_undated_records():
    ...
    state: dict[str, object] = {
        RETRY_KEYS_SESSION_KEY: {
            _scope("old", "focus", "Claim"): {
                "key": _key(),
                "notebook_id": "old",
                "scope_sha256": _scope("old", "focus", "Claim"),
    ...
    get_retry_key(state, thread_id="new", stage="focus", prompt="Claim", now=NOW)

    assert len(state[RETRY_KEYS_SESSION_KEY]) == 1
    assert _scope("new", "focus", "Claim") in state[RETRY_KEYS_SESSION_KEY]
