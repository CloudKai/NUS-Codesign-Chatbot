...
def test_ui_has_no_direct_infrastructure_or_model_sdk_imports() -> None:
    ...
    for path in (PROJECT_ROOT / "ui").rglob("*.py"):
        for target, _level in _top_level_imports(path):
            if target in banned or target.startswith(tuple(f"{name}." for name in banned)):
...
def test_student_store_public_and_dsql_occ_contracts_are_stable() -> None:
    ...
    expected_public = {
        "add_message", "add_source", "apply_phase_transition_decision",
        "claim_coach_request", "complete_coach_request", "consume_oauth_login_state",
        "create_phase_transition", "create_thread", "delete_source", "delete_thread",
        "fail_coach_request", "find_source_by_path", "get_messages",
        "get_messages_at_revision", "get_pending_phase_transition", "get_source",
        "get_thread", "get_user_by_cognito_sub", "get_user_by_id",
        "get_user_preferences", "list_sources", "list_threads",
        "lookup_completed_coach_request", "lookup_completed_or_recorded_coach_request",
        "persist_coach_turn", "ping", "rename_source", "resolve_phase_transition",
        "revise_conversation_from_user_message", "revise_user_message",
        "save_oauth_login_state", "select_learning_stage", "set_all_sources_selected",
        "set_source_selected", "try_resume_revision_result", "update_message",
        "update_thread", "update_user_preferences", "upsert_cognito_user",
    }
    actual_public = {
        name
    ...
    assert set(_OCC_WRITE_METHODS) == {
        "add_message", "add_source", "apply_phase_transition_decision",
        "claim_coach_request", "complete_coach_request", "consume_oauth_login_state",
        "create_phase_transition", "create_thread", "fail_coach_request",
        "persist_coach_turn", "rename_source", "resolve_phase_transition",
        "revise_conversation_from_user_message", "revise_user_message",
        "save_oauth_login_state", "select_learning_stage", "set_all_sources_selected",
        "set_source_selected", "update_message", "update_thread",
        "update_user_preferences", "upsert_cognito_user",
    }
...
def test_complete_fastapi_route_inventory_is_stable(tmp_path: Path) -> None:
    ...
    expected = {
        ...
        for method, path, name in (
            ...
            ("POST", "/api/v1/threads/{thread_id}/messages/{message_id}/revise", "revise_user_message"),
            ...
            ("PATCH", "/api/v1/threads/{thread_id}/sources/{source_id}", "update_source"),
            ("DELETE", "/api/v1/threads/{thread_id}/sources/{source_id}", "delete_source"),
            ("POST", "/api/v1/threads/{thread_id}/sources/select-all", "select_all_sources"),
            ("GET", "/api/v1/threads/{thread_id}/sources/{source_id}/content", "source_content"),
            ("POST", "/api/v1/threads/{thread_id}/sources/backfill-legacy", "backfill_legacy"),
            ("POST", "/api/v1/threads/{thread_id}/sources/sync-course-materials", "sync_course_materials"),
            ("GET", "/api/v1/threads/{thread_id}/learning-state", "learning_state"),
            ("POST", "/api/v1/threads/{thread_id}/learning-state/select-stage", "select_learning_stage"),
            ("GET", "/api/v1/threads/{thread_id}/phase-transitions/pending", "pending_transition"),
            ("POST", "/api/v1/threads/{thread_id}/phase-transitions/{transition_id}/resolve", "resolve_transition"),
