"""Keep the chat composer in a Cursor-style card with a capped textarea.

``sync_composer_layout`` injects DOM helpers that Streamlit does not expose for
sizing the composer card. Call once after rendering the composer widgets.
"""
...
def sync_composer_layout() -> None:
    """Apply composer card styling and cap the chat input to five rows."""
    components.html(
        """
<script>
(() => {
